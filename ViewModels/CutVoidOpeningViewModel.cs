...
{
    public sealed class CutVoidOpeningViewModel : ObservableObject, IDisposable
    {
        ...
        private readonly CutVoidIntersectManager manager = SmartToolController.Services.GetRequiredService<CutVoidIntersectManager>();


        public CutVoidOpeningViewModel()
        {
        ...
        private void CancelCallbackLogic()
        {
            ...
            lock (syncLocker)
            {
                ...
                catch (AggregateException)
                {
                    ...
                    {
                        Task.Delay(1000).ContinueWith((action) => LogManager.Warning("Task cansceled"));
                    }
                }
                finally
                {
        ...
        private async Task ExecuteSnoopCommandAsync()
        {
            ...
            ElementList = await RevitTask.RunAsync(app =>
            {
                ...
                if (0 < Properties.Settings.Default.OpennigFamilyId)
                {
                    ...
                    FamilySymbol symbol = CurrentDocument.GetElement(new ElementId(openingId)) as FamilySymbol;
                    if (symbol.IsValidObject && !symbol.IsActive)
                    {
                        ...
                        {
                            LogManager.Log("Activate symbol: " + symbol.FamilyName);
                        }
                    }
                }
                collection = manager.GetCollisionCommunicateElements();
                LogManager.Info($"Found collision {collection.Count()}");
