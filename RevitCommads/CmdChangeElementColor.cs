    ...
    public class CmdChangeElementColor : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                var r = sel.PickObject(ObjectType.Element, "Pick instance to change its colour");
                id = r.ElementId;
            }
        ...
        {
            var color = new Color(200, 100, 100);
        ...
        private void ChangeElementMaterial(Document doc, ElementId id)
        {
            ...
            if (null != e.Category)
            {
                ...
                var materials = new List<Material>(new FilteredElementCollector(doc)
                        .WhereElementIsNotElementType().OfClass(typeof(Material))
                        .ToElements().Where(m => m.Id.IntegerValue != im)
                        .Cast<Material>());
        ...
        private void PaintStairs(UIDocument uidoc, Material mat)
        {
            ...
            var pickedRef = sel.PickObject(ObjectType.PointOnElement, "Please select a Face");

            var elem = doc.GetElement(pickedRef);

            var geoObject = elem.GetGeometryObjectFromReference(pickedRef);
        ...
        private void PaintSelectedFace(UIDocument uidoc, Material mat)
        {
            ...
            var pickedRef = sel.PickObject(ObjectType.PointOnElement, "Please select a face to paint");
            ...
            using (var transaction = new Transaction(doc))
            {
                    ...
                    (int)BuiltInCategory.OST_Stairs))
                {
                    ...
                    foreach (var id in landings)
                    {
                        ...
                        var solids = GetElemSolids(land.get_Geometry(new Options()));
        ...
        ///     contained in the given instance geomety
        /// </summary>
                ...
                foreach (var geomObj in geomElem)
                    switch (geomObj)
                    {
                        ...
                        case GeometryInstance geomInst:
                            solids.AddRange(GetElemSolids(geomInst.GetInstanceGeometry()));
                            break;
                    }
